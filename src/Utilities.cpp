...
size_t GetFileSize(const char* path, Error& error) noexcept
{
    ...
    {
        Fail(FileSystemErrorCategory::Value::notFound, std::string("path \'") + path + "\' not found", __FILE__,
            __LINE__, error);
        return false;
    }
}
...
void CopyFile(const boost::filesystem::path& sourcePath, const boost::filesystem::path& targetPath, Error& error) noexcept
{
    ...
    if (ec.failed())
    {
        ...
        Fail(FileSystemErrorCategory::Value::generic, "", __FILE__, __LINE__, error);
    }
}
...
size_t ReadFile(const char* filename, char* buffer, size_t bufferSize, Error& error) noexcept
{
    ...
    try
    {
        ...
        {
            result = filesize;
            Fail(FileSystemErrorCategory::Value::bufferOverflow, error);
        }
    }
    catch (...)
    {
        Fail(FileSystemErrorCategory::Value::generic, std::string("unknown error for path \'") + filename + "\'",
            __FILE__, __LINE__, error);
    }
